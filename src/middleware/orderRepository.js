...
class OrderRepository {
  ...
  async save(order) {
    ...
    const result = await db.run(
      `INSERT INTO Orders (CustomerID, MerchantID, OrderStatus, TimeOrdered, TimeCompleted, TotalAmount) VALUES (?, ?, ?, ?, ?, ?)`,
      [
        order.customerId,
        order.merchantId,
        order.orderStatus,
    ...
    for (const item of order.orderItems) {
      await db.run(
        "INSERT INTO OrderItems (OrderID, ItemID, PriceAtPurchase, Quantity) VALUES(?, ?, ?, ?)",
        [orderId, item.itemId, item.priceAtPurchase, item.quantity],
      );
    }
  ...
  async update(order) {
    ...
    await db.run(`DELETE FROM OrderItems WHERE OrderID = ?`, [order.orderId]);

    for (const item of order.orderItems) {
      await db.run(
        `INSERT INTO OrderItems (OrderID, ItemID, PriceAtPurchase, Quantity)
                VALUES (?, ?, ?, ?)`,
        [order.orderId, item.itemId, item.priceAtPurchase, item.quantity],
      );
    }
  }
  ...
  async findByMerchantId(merchantId) {
    const db = await this.dbPromise;
    return await db.all(`SELECT * FROM Orders WHERE MerchantID = ?`, [
      merchantId,
    ]);
  }
