...
class MerchantRepository {
  ...
  async save(merchant, userId) {
    ...
    const result = await db.run(
      `INSERT INTO Merchants (UserID, MerchantName, MerchantAddress, Verified, StoreScore, Status) VALUES (?, ?, ?, ?, ?, ?)`,
      [
        userId,
        merchant.name,
        merchant.address,
  ...
  async deleteMenuItem(itemId) {
    const db = await this.dbPromise;
    await db.run(`DELETE FROM MenuItems Where ItemID = ?`, [itemId]);
  }
  ...
  async updateProfile(
    userId,
    merchantId,
    { firstName, lastName, address, bio, storeName },
  ) {
    const db = await this.dbPromise;
